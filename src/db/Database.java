...
public class Database {
    ...
    public void updatePatient(Patient p) {
        String sql = "UPDATE patients SET full_name=?, birth_date=?, gender=?, address=?, phone=?, " +
                "hospital_id=?, department_id=?, attending_doctor_id=?, diagnosis_id=?, status=? WHERE patient_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, p.getFullName());
            ps.setDate(2, java.sql.Date.valueOf(p.getBirthDate()));
            ps.setString(3, p.getGender());
    ...
    public void updateDoctor(Doctor d) {
        String sql = "UPDATE doctors SET full_name=?, birth_date=?, phone=?, inn=?, position_id=?, department_id=?, hospital_id=? WHERE doctor_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, d.getFullName());
            ps.setDate(2, java.sql.Date.valueOf(d.getBirthDate()));
            ps.setString(3, d.getPhone());
    ...
    public void updateDiagnosis(Diagnosis d) {
        String sql = "UPDATE diagnoses SET name=?, icd_code=?, description=? WHERE diagnosis_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, d.getName());
            ps.setString(2, d.getIcdCode());
            ps.setString(3, d.getDescription());
    ...
    public void updateHospital(Hospital h) {
        String sql = "UPDATE hospitals SET name=?, address=?, phone=?, inn=? WHERE hospital_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, h.getName());
            ps.setString(2, h.getAddress());
            ps.setString(3, h.getPhone());
    ...
    public void updateDepartment(Department d) {
        String sql = "UPDATE departments SET name=?, head_doctor=?, phone_extension=?, hospital_id=? WHERE department_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, d.getName());
            ps.setString(2, d.getHeadDoctor());
            ps.setString(3, d.getPhoneExtension());
    ...
    public void updatePosition(Position p) {
        String sql = "UPDATE positions SET title=?, salary=? WHERE position_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, p.getTitle());
            ps.setDouble(2, p.getSalary());
            ps.setInt(3, p.getId());
    ...
    public void updateRoom(Room r) {
        String sql = "UPDATE rooms SET room_number=?, department_id=?, beds=?, is_free=? WHERE room_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, r.getRoomNumber());
            ps.setInt(2, r.getDepartmentId());
            ps.setInt(3, r.getBeds());
    ...
    public void updateMedicine(Medicine m) {
        String sql = "UPDATE medicines SET name=?, price=?, quantity=? WHERE medicine_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, m.getName());
            ps.setDouble(2, m.getPrice());
            ps.setInt(3, m.getQuantity());
    ...
    public void updateAppointment(Appointment a) {
        String sql = "UPDATE appointments SET patient_id=?, doctor_id=?, appointment_date=?, complaints=?, status=? WHERE appointment_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, a.getPatientId());
            ps.setInt(2, a.getDoctorId());
            ps.setString(3, a.getAppointmentDate());
    ...
    public void updateMedicalRecord(MedicalRecord mr) {
        String sql = "UPDATE medical_records SET patient_id=?, diagnosis_id=?, doctor_id=?, record_date=?, treatment=?, notes=? WHERE record_id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, mr.getPatientId());
            ps.setInt(2, mr.getDiagnosisId());
            ps.setInt(3, mr.getDoctorId());
